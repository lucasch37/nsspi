...
func TestInterpretProgramOutput(t *testing.T) {
	tests := []struct {
		...
		{name: "arithmetic and precedence",
			source: "PROGRAM Test; BEGIN WRITELN(2 + 3 * 4); WRITELN((2 + 3) * 4); WRITELN(7 DIV 2); WRITELN(7 MOD 2) END.",
			want:   "14\n20\n3\n1\n"},
		{name: "real unary and string expressions",
			source: "PROGRAM Test; BEGIN WRITELN(7 / 2); WRITELN(-1.5); WRITELN('hello' + ' world') END.",
			want:   "3.5\n-1.5\nhello world\n"},
		{name: "conditionals",
			source: "PROGRAM Test; BEGIN IF 2 < 3 THEN WRITELN('yes') ELSE WRITELN('no'); IF 1 = 2 THEN WRITE('bad') ELSE WRITELN('ok') END.",
			want:   "yes\nok\n"},
...
func TestIntegerDivisionByZero(t *testing.T) {
	p, err := parser.NewParser(lexer.NewLexer("PROGRAM Test; BEGIN WRITELN(1 DIV 0) END."))
	if err != nil {
		t.Fatal(err)
	}
